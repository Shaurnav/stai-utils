...
def new_conversion(
    ...
    noise = torch.randn(latent_clean.shape, dtype=latent_clean.dtype).to(device)

    timesteps = torch.tensor([600] * latent_clean.shape[0])
    inverted_latents = diffuser_scheduler.add_noise(latent_clean, noise, timesteps)
    ...
    with torch.no_grad():
        converted_image = inferer.sample(
            ...
            diffusion_model=diffusion_model,
            scheduler=monai_scheduler,
            conditioning=new_condition_embedding,
...
def conversion(
    ...
    with autocast(device_type="cuda", dtype=torch.float16):
        ...
        # Use the last few steps of inverted latents
        inverted_latents = inverted_latents[-30]
    ...
    if save_path:
        ...
        original_age, original_sex = (
            original_condition[0, 0, 0].item(),
            original_condition[0, 0, 1].item(),
        )
        new_age, new_sex = new_condition[0, 0, 0].item(), new_condition[0, 0, 1].item()

        # Create filenames with age and sex information
        original_filename = (
            f"{save_path}_original_age{original_age:.1f}_sex{original_sex:.0f}.nii.gz"
        )
        converted_filename = (
            f"{save_path}_converted_age{new_age:.1f}_sex{new_sex:.0f}.nii.gz"
        )
...
def invert(
    ...
    diffuser_scheduler.set_timesteps(num_inference_steps, device="cuda")
    timesteps = reversed(diffuser_scheduler.timesteps).to(device)
    ...
    for i in tqdm(range(1, num_inference_steps), total=num_inference_steps - 1):
        ...
        noise_pred = diffusion_model(
            latent_model_input, timesteps=t, context=original_condition
        )

        current_t = max(0, t.item() - (1000 // num_inference_steps))
        next_t = t
        alpha_t = diffuser_scheduler.alphas_cumprod[current_t].to(device)

        diffuser_scheduler.alphas_cumprod = diffuser_scheduler.alphas_cumprod.to(device)
        alpha_t_next = diffuser_scheduler.alphas_cumprod[next_t]
