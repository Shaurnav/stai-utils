...
def get_hcpag_paths_and_metadata(root_dir):
    ...
    subject_dirs = [d for d in os.listdir(registered_dir) 
    ...
    for subject_dir in subject_dirs:
        ...
        assert os.path.exists(t1w_dir), f"HCP-Aging: T1w directory does not exist for subject {subject_id}"
        ...
        if os.path.exists(t1_file):
            subject_paths['t1_path'] = t1_file[len(root_dir):] # remove root_dir from path
        else:
            print(f"HCP-Aging: T1w file does not exist {t1_file}")
        if os.path.exists(t2_file):
            subject_paths['t2_path'] = t2_file[len(root_dir):] # remove root_dir from path
        else:
...
def get_hcpdev_paths_and_metadata(root_dir):
    ...
    for subject_dir in subject_dirs:
        ...
        # Look for T1 and T2 files in the T1w directory
        t1w_dir = os.path.join(subject_path, 'T1w')
        assert os.path.exists(t1w_dir), f"HCP-Development: T1w directory does not exist for subject {subject_id}"
...
def get_hcpya_paths_and_metadata(root_dir):
    ...
    for subject_dir in subject_dirs:
        ...
        # Look for T1 and T2 files in the T1w directory
        img_dir = os.path.join(subject_path, subject_id, f'unprocessed/{field_strength}/{t1_sequence}')

        t1_file = os.path.join(img_dir, f'{subject_id}_{field_strength}_{t1_sequence}_brain_mni_warped.nii.gz')
        t2_file = os.path.join(img_dir, f'{subject_id}_{field_strength}_{t2_sequence}_brain_mni_warped.nii.gz')
        # print(t1_file)
...
def get_openneuro_paths_and_metadata(root_dir):
    ...
    registered_dir = os.path.join(openneuro_root_dir, 'processed/Structural/registration')
    ...
    for subject_dir in subject_dirs:
        ...
        for ses_dir in os.listdir(subject_path):
            ...
            t1_file = os.path.join(img_dir, f'{subject_dir}_{ses_dir}_acq-MPRAGE_T1w_brain_mni_warped.nii.gz')
            t2_file = os.path.join(img_dir, f'{subject_dir}_{ses_dir}_acq-CUBE_T2w_brain_mni_warped.nii.gz')
...
def main():
    ...
    save_pkl(openneuro_paths, os.path.join(save_dir, "openneuro_relpaths_and_metadata.pkl"))
